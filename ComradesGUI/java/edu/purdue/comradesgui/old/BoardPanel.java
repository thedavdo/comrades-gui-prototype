...
public class BoardPanel extends JPanel implements MouseListener {
	...
	boolean WHITE_ALL_OUTLINE, BLACK_ALL_OUTLINE, WHITE_HALF_OUTLINE, BLACK_HALF_OUTLINE, WHITE_MATERIAL_OUTLINE,
			BLACK_MATERIAL_OUTLINE;
	int SIZE;
	BoardPosition POS;
	boolean REVERSE = false, SET_UP = false;
	String Name = null;
	String White = null, Black = null, Result = null;
	ArrayList<String> threeRepeatList;
	ArrayList<Integer> threeRepeatCount;
	public boolean threeRepeats = false;
	...
	public BoardPanel(BoardPosition pos) {
		...
		threeRepeatList = new ArrayList<String>();
		threeRepeatCount = new ArrayList<Integer>();
		threeRepeats = false;
	}
...
////////////////////////////////
	...
	public void mouseClicked(MouseEvent mou_evt) {
		if (CF.gameMode == 0 || CF.gameMode == 1)
...
//			MT.JumpToNode(MT.NOW.MainLineParent); // value for Instances
	...
	public void DrawCastling(Graphics G) {
		...
		if ((!REVERSE && POS.WhiteOOO) || (REVERSE && POS.BlackOO)) {
			G.drawLine(u * SIZE + SIZE / 2, 9 * SIZE + SIZE / 4, u * SIZE + 7, 9 * SIZE + SIZE / 4);
			G.drawLine(u * SIZE + SIZE / 2, 9 * SIZE + SIZE / 4 - 1, u * SIZE + 7, 9 * SIZE + SIZE / 4 - 1);
			G.drawLine(u * SIZE + 7, 9 * SIZE + SIZE / 4, u * SIZE + 12, 9 * SIZE + 5);
			G.drawLine(u * SIZE + 7, 9 * SIZE + SIZE / 4 - 1, u * SIZE + 12, 9 * SIZE + 6);
			G.drawLine(u * SIZE + 7, 9 * SIZE + SIZE / 4 - 1, u * SIZE + 12, 9 * SIZE + SIZE / 2 - 6);
			G.drawLine(u * SIZE + 7, 9 * SIZE + SIZE / 4, u * SIZE + 12, 9 * SIZE + SIZE / 2 - 7);
			if (POS.Chess960) {
				if (!REVERSE)
					G.fillArc(POS.White_QR_file * SIZE + 7, 9 * SIZE + 4, 7, 7, 0, 360);
				else
					G.fillArc((9 - POS.Black_KR_file) * SIZE + 7, 9 * SIZE + 4, 7, 7, 0, 360);
			}
		}
		if ((!REVERSE && POS.WhiteOO) || (REVERSE && POS.BlackOOO)) {
			G.drawLine(u * SIZE + SIZE / 2, 9 * SIZE + SIZE / 4, (u + 1) * SIZE - 7, 9 * SIZE + SIZE / 4);
			G.drawLine(u * SIZE + SIZE / 2, 9 * SIZE + SIZE / 4 - 1, (u + 1) * SIZE - 7, 9 * SIZE + SIZE / 4 - 1);
			G.drawLine((u + 1) * SIZE - 7, 9 * SIZE + SIZE / 4, (u + 1) * SIZE - 12, 9 * SIZE + 5);
			G.drawLine((u + 1) * SIZE - 7, 9 * SIZE + SIZE / 4 - 1, (u + 1) * SIZE - 12, 9 * SIZE + 6);
			G.drawLine((u + 1) * SIZE - 7, 9 * SIZE + SIZE / 4 - 1, (u + 1) * SIZE - 12, 9 * SIZE + SIZE / 2 - 6);
			G.drawLine((u + 1) * SIZE - 7, 9 * SIZE + SIZE / 4, (u + 1) * SIZE - 12, 9 * SIZE + SIZE / 2 - 7);
			if (POS.Chess960) {
				if (!REVERSE)
					G.fillArc(POS.White_KR_file * SIZE + 28, 9 * SIZE + 4, 7, 7, 0, 360);
				else
					G.fillArc((9 - POS.Black_QR_file) * SIZE + 28, 9 * SIZE + 4, 7, 7, 0, 360);
			}
		}
		if ((!REVERSE && POS.BlackOOO) || (REVERSE && POS.WhiteOO)) {
			G.drawLine(u * SIZE + SIZE / 2, 3 * SIZE / 4 + 1, u * SIZE + 7, 3 * SIZE / 4 + 1);
			G.drawLine(u * SIZE + SIZE / 2, 3 * SIZE / 4, u * SIZE + 7, 3 * SIZE / 4);
			G.drawLine(u * SIZE + 7, 3 * SIZE / 4, u * SIZE + 12, SIZE - 5);
			G.drawLine(u * SIZE + 7, 3 * SIZE / 4 + 1, u * SIZE + 12, SIZE - 6);
			G.drawLine(u * SIZE + 7, 3 * SIZE / 4 + 1, u * SIZE + 12, SIZE / 2 + 6);
			G.drawLine(u * SIZE + 7, 3 * SIZE / 4, u * SIZE + 12, SIZE / 2 + 7);
			if (POS.Chess960) {
				if (!REVERSE)
					G.fillArc(POS.White_QR_file * SIZE + 7, SIZE - 11, 7, 7, 0, 360);
				else
					G.fillArc((9 - POS.Black_KR_file) * SIZE + 7, SIZE - 11, 7, 7, 0, 360);
			}
		}
		if ((!REVERSE && POS.BlackOO) || (REVERSE && POS.WhiteOOO)) {
			G.drawLine(u * SIZE + SIZE / 2, 3 * SIZE / 4 + 1, (u + 1) * SIZE - 7, 3 * SIZE / 4 + 1);
			G.drawLine(u * SIZE + SIZE / 2, 3 * SIZE / 4, (u + 1) * SIZE - 7, 3 * SIZE / 4);
			G.drawLine((u + 1) * SIZE - 7, 3 * SIZE / 4, (u + 1) * SIZE - 12, SIZE - 5);
			G.drawLine((u + 1) * SIZE - 7, 3 * SIZE / 4 + 1, (u + 1) * SIZE - 12, SIZE - 6);
			G.drawLine((u + 1) * SIZE - 7, 3 * SIZE / 4 + 1, (u + 1) * SIZE - 12, SIZE / 2 + 6);
			G.drawLine((u + 1) * SIZE - 7, 3 * SIZE / 4, (u + 1) * SIZE - 12, SIZE / 2 + 7);
			if (POS.Chess960) {
				if (!REVERSE)
					G.fillArc(POS.White_KR_file * SIZE + 28, SIZE - 11, 7, 7, 0, 360);
				else
					G.fillArc((9 - POS.Black_QR_file) * SIZE + 28, SIZE - 11, 7, 7, 0, 360);
			}
		}
	}
	...
	public void DrawSquares(Graphics G, BoardPosition BP) {
		...
		G.drawLine(OFF_SET + SIZE * 8, OFF_SET + SIZE * 8, OFF_SET + SIZE * 8, OFF_SET - 1);
		G.drawLine(OFF_SET + SIZE * 8, OFF_SET + SIZE * 8, OFF_SET - 1, OFF_SET + SIZE * 8);
		if (POS.EnPassant != 0) {
			...
			if (REVERSE) {
				...
				if (POS.WTM)
					...
					G.fillArc(POS.EnPassant * SIZE + 8, 6 * SIZE + 8, SIZE - 17, SIZE - 17, 0, 360);
			}
		}
	}
	...
	public void clearThreeRepeats() {
		threeRepeatList.clear();
		threeRepeatCount.clear();
		threeRepeats = false;
	}
}

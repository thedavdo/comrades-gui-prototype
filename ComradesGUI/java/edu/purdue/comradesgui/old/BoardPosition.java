...
public class BoardPosition {
	...
	public void SetFEN(StringTokenizer ST) {
		...
		if (ST.hasMoreTokens()) {
			n = ST.nextToken();
			char ch = n.charAt(0);
			if ((new String("ABCDEFGHabcdefgh").indexOf(ch)) != -1) { // Chess960
	...
	public boolean AttacksWhite(int file, int rank) {
		...
		while ((rank - i >= 1) && (file + i <= 8)) // behind right
		{
			...
			if ((piece == 3 || (piece == 6 && i == 1) || piece == 5 || (piece == 1 && i == 1)))
				return true;
			break;
		}
		i = 1;
		...
		if ((((file + 2) <= 8) && ((rank + 1) <= 8) && ((file + 2) >= 1) && ((rank + 1) >= 1)
				&& (AT[(file + 2)][(rank + 1)] == 2)))
			return true;
		if ((((file + 1) <= 8) && ((rank + 2) <= 8) && ((file + 1) >= 1) && ((rank + 2) >= 1)
				&& (AT[(file + 1)][(rank + 2)] == 2)))
			return true;
		if ((((file - 2) <= 8) && ((rank - 1) <= 8) && ((file - 2) >= 1) && ((rank - 1) >= 1)
				&& (AT[(file - 2)][(rank - 1)] == 2)))
	...
	public boolean AttacksBlack(int file, int rank) {
		...
		if (file + 2 <= 8 && rank + 1 <= 8 && file + 2 >= 1 && rank + 1 >= 1 && AT[file + 2][rank + 1] == -2)
			return true;
		if ((((file + 1) <= 8) && ((rank + 2) <= 8) && ((file + 1) >= 1) && ((rank + 2) >= 1)
				&& (AT[(file + 1)][(rank + 2)] == -2)))
			return true;
	...
	public void AddMove(int from_file, int from_rank, int to_file, int to_rank, int type) {
		int v = (from_file - 1) + ((from_rank - 1) << 3) + ((to_file - 1) << 6) + ((to_rank - 1) << 9) + (type << 12);
	...
	public void LegalWhite(int from_file, int from_rank, int to_file, int to_rank, int type) {
	...
	public void LegalBlack(int from_file, int from_rank, int to_file, int to_rank, int type) {
		int piece = AT[to_file][to_rank];
		if (Chess960 && type == 6) {
	...
	public boolean CheckMove(int from_file, int from_rank, int to_file, int to_rank) {
		if (WTM) {
	...
	public void Handle_Chess960_White_Castle() {
		if (WhiteOOO && AT[White_QR_file][1] == 4 && AT[Chess960_WK_File][1] == 6) {
			boolean b = true;
		...
		if (WhiteOO && AT[White_KR_file][1] == 4 && AT[Chess960_WK_File][1] == 6) {
			boolean b = true;
	...
	public void Handle_Chess960_Black_Castle() {
		if (BlackOOO && AT[Black_QR_file][8] == -4 && AT[Chess960_BK_File][8] == -6) {
			boolean b = true;
		...
		if (BlackOO && AT[Black_KR_file][8] == -4 && AT[Chess960_BK_File][8] == -6) {
			boolean b = true;
	...
	public void GenerateAllMoves() {
		...
		if (!Chess960) {
			if (WTM) {
				if (WhiteOOO && AT[1][1] == 4 && AT[5][1] == 6 && AT[2][1] == 0 && AT[3][1] == 0 && AT[4][1] == 0)
	...
	public void MakeNormal() {
		...
		if (EnPassant != 0) {
			if (WTM && AT[EnPassant + 1][5] != 1 && AT[EnPassant - 1][5] != 1)
				EnPassant = 0;
	...
	public void MakeNotation() {
		...
		for (int i = 0; i < COUNT_OF_LEGAL_MOVES; i++) {
			...
			if (type == 6)
				...
				int from_file = 1 + (move & 7), from_rank = 1 + ((move >> 3) & 7), to_file = 1 + ((move >> 6) & 7),
						to_rank = 1 + ((move >> 9) & 7);
	...
	public boolean IsOK() {
		...
		if (COUNT[3] > 9 || COUNT[9] > 9 || COUNT[4] > 10 || COUNT[8] > 10)
			return false;
		for (file = 1; file <= 8; file++)
			if (AT[file][1] == 1 || AT[file][1] == -1 || AT[file][8] == 1 || AT[file][8] == -1)
				return false;
		if (WTM && AttacksWhite(BLACK_KingFile, BLACK_KingRank))
			return false;
		if (!WTM && AttacksBlack(WHITE_KingFile, WHITE_KingRank))
	...
	public String GetDirect(int MOVE) {
		...
		int from_file = 1 + (mv & 7), from_rank = 1 + ((mv >> 3) & 7), to_file = 1 + ((mv >> 6) & 7),
				to_rank = 1 + ((mv >> 9) & 7);
	...
	public void MakeMove32(int mv, String S) {
		...
		int OoOoO = (WhiteOO ? 1 : 0) | (WhiteOOO ? 2 : 0) | (BlackOO ? 4 : 0) | (BlackOOO ? 8 : 0);
		int ct = (EnPassant > 0) ? (EnPassant + 110) : ((ReversibleCount > 100) ? 100 : ReversibleCount);
		if (MOVE_TREE != null)
	...
	public boolean is50MovesRuleReached() {
		if (ReversibleCount >= 100) {
			return true;
		}
	...
	public String GetFENwithoutMoves() {
		String R = GetPositionalFEN();
		if (WTM)
			R += " w ";
		else

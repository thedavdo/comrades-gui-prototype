...
public class MoveTree extends JTree // JPanel ?
{
	...
	public String SubPGN(ComradesNode N, int ht, boolean b) {
		...
		if (c > 1)
			for (int i = 1; i < c; i++) {
				S += " " + "(" + (ht / 2) + ((ht % 2) == 0 ? ". " : "... ") + N.getChildAt(i).toString()
	...
	public String EmitPGNTree() {
		...
		if (BP.Chess960)
			S += "[Variant \"Chess960\"]\n";
		if (!START_FEN.equals("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"))
			S += "[FEN \"" + START_FEN + "\"]\n";
		S += "\n" + LineSplit(SubPGN(ROOT, 2 * MOVE_NUMBER + (WTM ? 0 : 1), false) + " " + BP.PANEL.Result, 75);
		return S + "\n";
	}
	...
	public String EmitPGNMainLine() {
		...
		if (!WTM)
			S += "" + MOVE_NUMBER + "... ";
		S += LineSplit(MainLinePGN(2 * MOVE_NUMBER + (WTM ? 0 : 1)) + " " + BP.PANEL.Result, 75);
		return S + "\n";
	}
